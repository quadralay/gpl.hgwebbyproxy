...
class HGWebByProxy(object):
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            if ("HTTP_PROXY_HG_PATH" in environ) and (
                "HTTP_PROXY_HG_SCRIPT_NAME" in environ
            ):
                ...
                __logger__.debug(
                    "redirect_app: proxy_hg_script_name == %s" % proxy_hg_script_name
                )

                __logger__.debug("redirect_app: 3")
                __logger__.debug("proxy_hg_app: PATH_INFO == %s" % environ["PATH_INFO"])
                __logger__.debug(
                    "proxy_hg_app: SCRIPT_NAME == %s" % environ["SCRIPT_NAME"]
                )
